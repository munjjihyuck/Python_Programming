...
# result = {i: i**2 for i in range(1, 11) if i % 2 != 0}    # 같은 결과
print(result)
...
# 2) dict 컴프리헨션
count = {i: words.count(i) for i in set(words)}
print(count)
...
# 2) dict 컴프리헨션
stock.update({item: stock.get(item, 0) + qty for itme, qty in incoming.items()})
print(stock)

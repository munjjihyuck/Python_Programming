...
subjects = ["국어", "수학", "영어", "과학"]  # 개수 안맞을땐 없앰
scores = [80, 90, 95]
print(*zip(subjects, scores))

a, b, c = zip(subjects, scores)  # zip(a, b, strict=true) 이거있으면 개수 안맞을때 오류
print(a, b, c)
...
# result = [i for i in range(1, 101) if not i % 3 or not i % 5 == 0]
...
# result = [i for i in words if w.find("n") >= 0]
...
result = [sum(i) / 3 for i in zip(std1, std2, std3)]
